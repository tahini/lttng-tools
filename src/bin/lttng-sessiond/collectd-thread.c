...
{
	int ret, fd_r = 0;
	struct registration_thread_handle *register_thread_handle = data;
	 ...
	 */
	sem_wait(&register_thread_handle->registration_thread_ready);
...
static bool shutdown_collectd_thread(void *data)
{
	return true;
}

static void cleanup_collectd_thread(void *data)
{
...
bool launch_collectd_thread(struct registration_thread_handle *registration_thread_handle)
{
	...
	thread = lttng_thread_create("Collectd",
			thread_manage_collectd,
			shutdown_collectd_thread,
			cleanup_collectd_thread,
